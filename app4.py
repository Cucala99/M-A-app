...
df = pd.read_excel('/Users/antoniocucala/Desktop/Capstone Database.xlsx')
...
if choice == "Valuation Calculator":
    ...
    valuation_method = st.selectbox("Select Valuation Method", ["DCF", "P/E Ratio", "Historical Data Analysis"])
    ...
    if valuation_method == "DCF":
    ...
    elif valuation_method == "Historical Data Analysis":
        st.title("Historical Data Analysis")

        # Company selection
        company_name_historical = st.selectbox("Select Company for Historical Data Analysis", df["Name"].unique())
        
        # Filter dataframe for the selected company
        company_data = df[df['Name'] == company_name_historical]
        ...
        for column in ['Operating result (thousand EUR) ', 'Net profit (thousand EUR)', 'EPS']:
            plt.plot(year_list, filtered_data[column + ' ' + selected_year], label=column)
        plt.xlabel("Year")
        plt.ylabel("Value")
        plt.title("Historical Financial Performance")
        plt.legend()
        st.pyplot()
